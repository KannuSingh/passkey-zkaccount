...
export class Passkey {
  ...
  static async create({ appName, username, email, yubikeyOnly }: { appName: string, username: string, email?: string, yubikeyOnly?: boolean }): Promise<PasskeyCredentialResponse> {
    logger.debug(`(🪪,ℹ️) Creating credential for ${username}`);
    try {
  ...
  static async getPublicKeyXYCoordinate(publicKey: CryptoKey):Promise<[string, string]>{
    let jwkKey;
    try {
  ...
  static async getPasskeySignatureData(challenge:string, allowCredentials?: PublicKeyCredentialDescriptor[] ){
    try {
      logger.debug('(🪪,ℹ️) Obtaining credentials');
      const challengeBuf = this.hex2buf(challenge.substring(2))
      logger.debug('(🪄,ℹ️) Challenge', challenge);
      logger.debug('(🪄,ℹ️) Challenge (base64)', this.toBase64url(challengeBuf));
      logger.debug('(🪄,ℹ️) Challenge (hex)', this.buf2hex(challengeBuf));
      ...
      return {  authenticatorData:"",
        ...
        r:"",s:"", error: PASSKEY_ERRORS.UNABLE_TO_RETRIEVE_CREDENTIAL };
    }
    ...
    // if (assertion) {
      ...
      logger.debug('(👤,ℹ️), clientDataJSON (parsed)', JSON.stringify(obtainedClientDataJSON));
      logger.debug('(👤,ℹ️), challenge (from clientDataJSON)', this.buf2hex(this.parseBase64url(obtainedClientDataJSON.challenge)));

      const authenticatorDataString = "0x"+this.buf2hex(authenticatorData)
      ...
      logger.info({ authenticatorData:authenticatorDataString,
        ...
        r:rValue,s:sValue })
      return { authenticatorData:authenticatorDataString,
        requireUserVerification,
        clientDataJson:clientDataJSONString,
        challengeLocation,
        responseTypeLocation,
        r:rValue,s:sValue, error: null }
    // }
  }
  ...
  static normalizeSignature(signature:ArrayBuffer): { r: bigint; s: bigint }{
    ...
    var r = BigInt("0x"+this.buf2hex(usignature.slice(rStart, rEnd)));
    var s = BigInt("0x"+this.buf2hex(usignature.slice(sStart)));
